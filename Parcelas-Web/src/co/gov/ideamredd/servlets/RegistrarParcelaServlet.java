...
import co.gov.ideamredd.dao.ObtenerParcelaConsulta;
import co.gov.ideamredd.dao.RegistrarParcela;
import co.gov.ideamredd.entities.Contacto;
import co.gov.ideamredd.entities.Organizacion;
import co.gov.ideamredd.util.Util;

public class RegistrarParcelaServlet extends HttpServlet {

    private static final long serialVersionUID = 1L;

    @EJB
    private RegistrarParcela parcela;
    @EJB
...
//	    .getBundle(archivoParcela);
    ...
    String ubicacionArchivo = ObtenerParcelaConsulta
	    .getRutaBD("imagenes_parcela");
    // Util.obtenerClave("parcela.ruta.archivo.fotos",par);
    UploadFile file;
	...
	try {
		...
		parcela.setUna_Ubicacion(ObtenerParcelaConsulta
			    .getRutaBD("imagenes_parcela"));
	    }
	    ...
	    if (mrequest.getParameter("isInv") != null
		    && mrequest.getParameter("isInv").equals(
			    Constantes.registraOtroContacto))
		registrarOtrosContactosParcela(Constantes.claveInvestigador,
			Constantes.opcionInv);
	    if ((mrequest.getParameter("isCol") != null || mrequest
		    .getParameter("isEnc") != null)
		    && (mrequest.getParameter("isCol").equals(
			    Constantes.registraOtroContacto) || mrequest
			    .getParameter("isEnc").equals(
				    Constantes.registraOtroContacto))) {
	    ...
	    if (mrequest.getParameter("isEnc") != null
		    && mrequest.getParameter("isBri").equals(
			    Constantes.registraOtroContacto))
		registrarOtrosContactosParcela(Constantes.claveBrigadista,
			Constantes.opcionBri);
	    if (mrequest.getParameter("isEnc") != null
		    && mrequest.getParameter("isSup").equals(
			    Constantes.registraOtroContacto))
	...
	if (mrequest.getParameter("sel" + clave).equals("0")) {
		...
		parcela.getContactos().put(
		    ...
		    Constantes.registrarContacto)) {
		if (mrequest.getParameter("tipoCont" + clave).equals(
			Constantes.personaNatural)) {
		    contacto = new Contacto();
		    contacto.setNombre(mrequest.getParameter("nomCon" + clave));
		    contacto.setTelefono(mrequest
			    .getParameter("fijCon" + clave));
