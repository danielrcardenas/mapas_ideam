...
<title>Registrar Inventario de Suelo</title>
...
<body>
	<div id="contenedor">
		...
		<div id="head">
			<h1>Registrar Inventario de Suelo</h1>
		</div>
		...
		<div id="cuerpo">
			<form action="<%=basePath%>registrarInventarioSueloServlet"
				method="post" name="modificaF" enctype="multipart/form-data"
				onsubmit="return verifiRegistro()">
				<%
					iSuelo = (InventarioSuelo) invSuelos.get(indice);
					out.print("<input type=\"hidden\" name=\"parcelaID\" value=\""
							+ iSuelo.getParcelaId() + "\" >");
					out.print("<input type=\"hidden\" name=\"parcelaNombre\" value=\""
							+ iSuelo.getParcela() + "\" >"); 
				%>
				<%
					out.print("<h4>Metodologia:</h4>");
				%>
				...
				<input type="text" name="newProfToma" id="newProfToma"
					onkeypress="return valideValNum(event)">
				<%
					out.print("</p>");
				%>
				<h4>Textura:</h4>
				<input type="text" name="newTextura" id="newTextura">
				<%
					out.print("</p>");
				%>
				<h4>Densidad Aparente:</h4>
				<input type="text" name="newDensAparente" id="newDensAparente"
					onkeypress="return valideValNum(event)">
				<%
					out.print("</p>");
				%>
				<h4>Flujo CO2:</h4>
				<input type="text" name="newFlujoCO2" id="newFlujoCO2"
					onkeypress="return valideValNum(event)">
				<%
					out.print("</p>");
				%>
				...
				<input id="newFechaTomaDatos" name="newFechaTomaDatos"
					onkeypress="return noKeyData(event);" value="">
				<%
					out.print("</p>");
				%>
				<input type="submit" value="Registrar"> <input type="button"
					value="Cancelar" onclick="javascript:history.back()">
			</form>
		</div>
