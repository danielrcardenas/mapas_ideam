...
import co.gov.ideamredd.entities.Modulos;
import co.gov.ideamredd.entities.Permisos;
import co.gov.ideamredd.entities.PermisosRol;  
import co.gov.ideamredd.entities.Rol;
...
public class CargaDatosInicial {
	...
	public static ArrayList<Rol> cargarRoles() {
		...
		try {
			...
			CallableStatement consultaRoles = conn
					.prepareCall("{call RED_PK_USUARIOS.consultarTodosRol(?)}");
			consultaRoles.registerOutParameter("un_resultado",
					OracleTypes.CURSOR);
			consultaRoles.execute();
			ResultSet r = (ResultSet) consultaRoles
	...
	public static Rol consultaRol(Integer idRol) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaRoles = conn
					.prepareCall("{call RED_PK_USUARIOS.consultarRol(?,?)}");
	...
	public static ArrayList<Modulos> consultarModulos(){
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaModulos = conn
					.prepareCall("{call RED_PK_USUARIOS.consultarModulos(?)}");
			consultaModulos.registerOutParameter("un_resultado",
					OracleTypes.CURSOR);
			consultaModulos.execute();
			ResultSet r = (ResultSet) consultaModulos
					.getObject("un_resultado");
			while (r.next()) {
	...
	public static ArrayList<PermisosRol> consultarPermisosRol(Integer idRol) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaPermisosRol = conn
					.prepareCall("{call RED_PK_USUARIOS.consultarRolPermisos(?,?)}");
			consultaPermisosRol.setInt("un_rol", idRol);
			consultaPermisosRol.registerOutParameter("un_resultado",
					OracleTypes.CURSOR);
			consultaPermisosRol.execute();
			ResultSet r = (ResultSet) consultaPermisosRol
					.getObject("un_resultado");
			while (r.next()) {
	...
	public static ArrayList<Permisos> consultarPermisos() {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaPermisos = conn
					.prepareCall("{call RED_PK_USUARIOS.consultarPermisos(?)}");
			consultaPermisos.registerOutParameter("un_resultado",
					OracleTypes.CURSOR);
			consultaPermisos.execute();
			ResultSet r = (ResultSet) consultaPermisos
					.getObject("un_resultado");
			while (r.next()) {
