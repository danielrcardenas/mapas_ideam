...
public class ConsultaProyecto {
	...
	public static String[] consultarGeoProyecto(Integer idProyecto) {
		...
		try {
			...
			ResultSet resultSet = (ResultSet) consultarPaisProyecto
					.getObject("un_Resultado");
			while (resultSet.next()) {
			...
			for (int xy = 0; xy < coordAux.size(); xy++) {
				...
				if(xy+1<coordAux.size()){coordenadas[1] = coordenadas[1]+",";}
			}
			conn.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static ArrayList<DocumentosAsociados> consultarDocumentosProyecto(
		...
		try {
			...
			while (resultSet.next()) {
				...
				documentosAsociados.setUn_NombreDocumento(resultSet
						.getString(2));
				documentosAsociados.setUna_Ruta(resultSet.getString(3));
				documentosAsociados.setUna_FechaIngresoDocumento(resultSet
						.getTimestamp(4));
