...
package co.gov.ideamredd.web.reportes.dao;
...
import co.gov.ideamredd.reportes.entities.CAR;
import co.gov.ideamredd.reportes.entities.CarDepto;
import co.gov.ideamredd.reportes.entities.Depto;
import co.gov.ideamredd.reportes.entities.MetodologiaBiomasa;
import co.gov.ideamredd.reportes.entities.Municipios;
import co.gov.ideamredd.reportes.entities.Parcela;
import co.gov.ideamredd.reportes.entities.Reportes;
import co.gov.ideamredd.reportes.entities.Temporalidad;

import co.gov.ideamredd.util.SMBC_Log;

/**
 * clase usuada para cargar datos iniciales de fromularios de reportes.
 */
public class CargaDatosSelect {
	...
	public static String getPaises() {
		...
		try {
			...
			while (resultSet.next()) {
				pais += "<option value=\"" + resultSet.getObject(1) + "\">" + resultSet.getObject(2) + "</option>\n";
			}
	...
	public static String getDepartamentos() {
		...
		try {
			...
			while (resultSet.next()) {
				departamentos += "<option value=\"" + resultSet.getObject(2) + "\">" + resultSet.getString(3) + "</option>\n";
	...
	public static String getMunicipios() {
		...
		try {
			...
			ResultSet resultSet = (ResultSet) consultaDepartamento.getObject("un_Resultado");
			while (resultSet.next()) {
				departamentos += "<option value=\"" + resultSet.getObject(1) + "\">" + resultSet.getObject(2) + "</option>\n";
				// System.out.println("<option value=\""+resultSet.getObject(1)+"\">"+resultSet.getObject(2)+"</option>");
			}
		...
		try {
			...
			while (resultSet.next()) {
				if (!resultSet.getString(2).contains("LOCALIDAD")) {
					...
					municipios.setConsecutivo(Integer.valueOf(resultSet.getObject(5).toString()));
	...
	public static String getTipoDocumento() {
		...
		try {
			...
			while (resultSet.next()) {
				tipoDocumento += "<option value=\"" + resultSet.getObject(1) + "\">" + resultSet.getObject(2) + "</option>\n";
			}
	...
	public static String getTipoPersona() {
		...
		try {
			...
			while (resultSet.next()) {
				tipoPersona += "<option value=\"" + resultSet.getObject(1) + "\">" + resultSet.getObject(2) + "</option>\n";
			}
	...
	public static String getTipoReporte(String idReportes) {
		...
		try {
			...
			if (idReportes.equals("bosques")) {
				consultaTipoReporte = conn.prepareCall("{call RED_PK_TABLASTIPO.TipoReporte_ConsultaBosques(?, ?)}");
			}
			else {
				consultaTipoReporte = conn.prepareCall("{call RED_PK_TABLASTIPO.TipoReporte_ConsultaCarbono(?, ?)}");
			}
	...
	public static String getDivisionTerritorio(String tipoReporte) {

		String divTerritorio = "";
		try {
			log = SMBC_Log.Log(CargaDatosSelect.class);
			conn = dataSource.getConnection();
			CallableStatement consultaDivTerritorio = null;
			
			if (!tipoReporte.equals("5")) {
				consultaDivTerritorio = conn.prepareCall("{call RED_PK_TABLASTIPO.DivTerritorio_Consulta(?, ?)}");
			}
			else {
				consultaDivTerritorio = conn.prepareCall("{call RED_PK_TABLASTIPO.DivTerritorio_Consulta_sincon(?, ?)}");
			}
	...
	public static ArrayList<Reportes> getPeriodos() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				reportes.setConsecutivo(resultSet.getInt("RPRT_CONSECUTIVO"));
				reportes.setFechaGeneracion(resultSet.getDate("RPRT_FECHAGENERACION"));
				reportes.setDivision(resultSet.getInt("RPRT_DIVISIONTERRITORIO"));
				reportes.setPeriodoUno(resultSet.getInt("RPRT_PERIODOUNO"));
				reportes.setPeriodoDos(resultSet.getInt("RPRT_PERIODODOS"));
				reportes.setTipoReporte(resultSet.getInt("RPRT_TIPOREPORTE"));
	...
	public static ArrayList<Reportes> getPeriodosBC() {
		
		ArrayList<Reportes> listaReportes = new ArrayList<Reportes>();
		try {
			log = SMBC_Log.Log(CargaDatosSelect.class);
			conn = dataSource.getConnection();
			CallableStatement consultaPeriodosReportes = conn.prepareCall("{call RED_PK_TABLASTIPO.PeriodosReportes_ConsultaBC(?)}");
			consultaPeriodosReportes.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultaPeriodosReportes.execute();
			ResultSet resultSet = (ResultSet) consultaPeriodosReportes.getObject("un_Resultado");
			while (resultSet.next()) {
