...
import co.gov.ideamredd.admin.dao.ModificarLicencia;
...
/**
 * Servlet usado para modificar una licencia
 */
public class ModificarLicenciaServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (ServletFileUpload.isMultipartContent(req)) {
			try {
				...
				for (FileItem item : multiparts) {
					...
					}else if (item.getFieldName().equals("hidConsecutivo")) {
						idLicencia = Integer.parseInt(getStringFromInputStream(item.getInputStream()));
					}else if (item.getFieldName().equals("hidHayDoc")) {
						hayDoc = getStringFromInputStream(item.getInputStream());
					}

					if(hayDoc.equals("Yes"))
					if (!item.isFormField()) {
		...
		mli.guardarDatos(idLicencia,nombreLicencia, descripcion, activa);

		req.getSession().setAttribute("nombreLicencia", nombreLicencia);
		resp.sendRedirect("admin/consultaLicenciasUso.jsp");
