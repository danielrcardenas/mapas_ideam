...
import co.gov.ideamredd.usuario.entities.CAR;
import co.gov.ideamredd.usuario.entities.CarDepto;
import co.gov.ideamredd.usuario.entities.Depto;
import co.gov.ideamredd.usuario.entities.MetodologiaBiomasa;
import co.gov.ideamredd.usuario.entities.Municipios;
import co.gov.ideamredd.usuario.entities.Parcela;
import co.gov.ideamredd.usuario.entities.Reportes;
import co.gov.ideamredd.usuario.entities.Temporalidad;
import co.gov.ideamredd.mbc.conexion.ConexionBD;
import co.gov.ideamredd.util.SMBC_Log;

/**
 * clase usuada para cargar datos iniciales de fromularios de usuarios.
 */
public class CargaDatosSelect {
	...
	public static String getPaises() {
		...
		try {
			...
			CallableStatement consultaPais = conn
					.prepareCall("{call RED_PK_TABLASTIPO.Paices_Consulta(?)}");
			...
			while (resultSet.next()) {
				pais += "<option id=\"pais" + resultSet.getObject(1)
	...
	public static String getDepartamentos() {
		...
		try {
			...
			ResultSet resultSet = (ResultSet) consultaDepartamento
					.getObject("un_Resultado");
			while (resultSet.next()) {
				departamentos += "<option id=\"depto" + resultSet.getObject(1)
						+ "\" value=\"" + resultSet.getObject(1) + "\">"
	...
	public static String getMunicipios() {
		...
		try {
			...
			ResultSet resultSet = (ResultSet) consultaDepartamento
					.getObject("un_Resultado");
			while (resultSet.next()) {
				departamentos += "<option id=\"muni" + resultSet.getObject(1)
						+ "\" value=\"" + resultSet.getObject(1)
						+ "\">" + resultSet.getObject(2) + "</option>\n";
				// System.out.println("<option value=\""+resultSet.getObject(1)+"\">"+resultSet.getObject(2)+"</option>");
			}
	...
	public static String getTipoDocumento() {
		...
		try {
			...
			ResultSet resultSet = (ResultSet) consultaTipoDocumento
					.getObject("un_Resultado");
			while (resultSet.next()) {
				tipoDocumento += "<option id=\"tipoDoc" + resultSet.getInt(1)
	...
	public static String getTipoPersona() {
		...
		try {
			...
			ResultSet resultSet = (ResultSet) consultaTipoPersona
					.getObject("un_Resultado");
			while (resultSet.next()) {
				tipoPersona += "<option id=\"tipoPOp" + resultSet.getObject(1)
						+ "\" value=\"" + resultSet.getObject(1) + "\">"
						+ resultSet.getObject(2) + "</option>\n";
			}
			resultSet.close();
	...
	public static String rolesUsuario() {
		...
		try {
			...
			conn = dataSource.getConnection();
			CallableStatement consultaRoles = conn
					.prepareCall("{call RED_PK_USUARIOS.consultarRolesUsuario(?)}");
			consultaRoles.registerOutParameter("un_Resultado",
					OracleTypes.CURSOR);
			consultaRoles.execute();
			ResultSet resultSet = (ResultSet) consultaRoles
					.getObject("un_Resultado");
			while (resultSet.next()) {
					roles += "<option id=\"chkRol"+resultSet.getObject(1)+"\" value=\"" + resultSet.getObject(1) + "\">"
							+ (resultSet.getString(4).replace('_', ' ')).toUpperCase()
							+ "</option>";
			}
