...
if(usuario !=null) {
	diccionarioPermisos = ControlPermisos.consultaPermisos(usuario.getRolId());
		...
		ArrayList<String> rolesLista = (ArrayList<String>) CargaDatosSelect.rolesUsuarioLista();
		
		
		Integer cargadoCorrecto = (Integer)request.getSession().getAttribute("cargadoCorrecto");
		
		if(cargadoCorrecto==null){
			cargadoCorrecto=-1;
		}
...
<%
for(int x=0;x<rolesLista.size();x++)
{%>
	listaRoles[<%=x%>]='<%=rolesLista.get(x)%>';
<%}
%>
	...
	function validarDatosForm(){
		...
		var archivoDatos=document.getElementById("archivoDatos").value;
		
		if(archivoDatos.indexOf('.zip')==-1 || archivoDatos==null || archivoDatos==''){
			resultado=false;
		}
		
		if (!resultado) {
			alert("El archivo es obligatorio y debe estar comprimido como .zip");
		}
		
		return resultado;
	}
	
	function descargaEjemploAlertas(){
		document.getElementById('formEjemploAlertas').submit();
	}
...
<body class='sidebarlast front' onMouseMove="takeCoordenadas(event);" onmouseover="popUpUsuariosAux()">
				...
				<div id="main-inner" class="section-wrapper">
					<div class="section">
						<div class="section-inner clearfix fondoformulario">
							...
							<h2 class="titulo_naranja"><%=msj.getString("Cargar_Datos_Alertas_Tempranas") %></h2>

							<div id="content">
								<div class="content-inner">

									<form method="POST" enctype="multipart/form-data" action="/MonitoreoBC-WEB/cargarDatosReportesServlet" id="formImagenUsuario" name="formImagenUsuario" onsubmit="return validarDatosForm()">

									<div id="block-consulta-reportes" class="block-gray block">
									
										<h2><%=msj.getString("formulario_de_cargue") %></h2>

										
										<div style="margin-left: 20px">
										
										<label for="exampleInputEmail1"><%=msj.getString("caracteristicas_archivo_alertas") %>:</label>
										<label for="exampleInputEmail1" style="padding-left: 50px; color: red; cursor: pointer;" onclick="descargaEjemploAlertas()">-<%=msj.getString("ejemplo_archivo_zip_alertas") %></label><br>
										
										<%if(cargadoCorrecto!=-1){
												if(cargadoCorrecto==1){
													request.getSession().setAttribute("cargadoCorrecto",-1);
										%>
											<label style="font-size: 14px;color: green;"><%=msj.getString("Carga_realizada_correctamente") %></label>
										<%}else{
												if(cargadoCorrecto==0){
												request.getSession().setAttribute("cargadoCorrecto",-1);%>
												<label style="font-size: 14px;color: red;"><%=msj.getString("problema_cargue_archivo_alertas")%>.</label>
										<%		}
											}
										}%>
										
										<label for="exampleInputEmail1"><%=msj.getString("archivo_de_datos") %>:<span class="obligatorio">*</span></label>
										
										
										<input type="file" name="archivoDatos" id="archivoDatos" style="width: 280px">
										
										</div>
										<br>
										<div class="form-actions">
										<input type="submit" value="<%=msj.getString("subir") %>">
										<input type="button"
	...
	<form action="/MonitoreoBC-WEB/descargaEjemploAlertas" method="post" name="formEjemploAlertas"
		id="formEjemploAlertas">
	</form>
