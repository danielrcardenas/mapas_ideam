...
if(usuario !=null) {
	diccionarioPermisos = ControlPermisos.consultaPermisos(usuario.getRolId());
		...
		ArrayList<String> licenciasLista = (ArrayList<String>) CargaDatosSelect.licenciasLista();
...
<script>
var listaLicencias=Array(<%=licenciasLista.size()%>);

<%
for(int x=0;x<licenciasLista.size();x++)
{%>
listaLicencias[<%=x%>]='<%=licenciasLista.get(x)%>';
<%}
%>
	...
	function validarDatosForm(){
		...
		nombreLicencia=nombreLicencia.replace(/\s/g,"_").toUpperCase();
		
		var mensaje="";
		
		if(nombreLicencia == ""){resultado=false;mensaje=mensaje+"-Nombre de la Licencia\n";}
		if($.inArray(nombreLicencia, listaLicencias)!=-1){
			resultado=false;mensaje=mensaje+"-El nombre de licencia ya existe\n";
		}
// 		if(tipoNoticia == "-1"){resultado=false;mensaje=mensaje+"-Tipo de Noticia\n";}
		...
		if(docLicencia == ""){resultado=false; mensaje=mensaje+"-Documento\n";}
		if(docLicencia != ""){
							...
							<div id="content">
								<div class="content-inner">
									...
									<div id="block-consulta-reportes" class="block-gray block">
										...
										<form method="POST" enctype="multipart/form-data" action="/MonitoreoBC-WEB/registrarLicenciaServlet" 
										...
										<input type="text" name="nombreLicencia" id="nombreLicencia" style="width: 280px">
										<br><br>
...
<!-- 										<br> -->
										...
										<label for="exampleInputEmail1">Descripcion (Breve):<span class="obligatorio">*</span></label>
										...
										name="licenciaActiva" id="licenciaActiva" checked="checked">Activar Licencia<span class="obligatorio">*</span></label>
										<br>
										
										<label for="exampleInputEmail1">Adjuntar Documento de licencia(PDF):<span class="obligatorio">*</span></label>
