...
$(document).ready(function () {
	...
	function resultadosContactos(tamanho, principal, contenedor) {
		...
		results.innerText = '<%=msj.getString("registros_encontrados")%>:' + tamanho;
		results.innerHTML = '<%=msj.getString("registros_encontrados")%>:' + tamanho;
	...
	function enviar() {
		if (enviando) {
			$("#div_resultado_envio").html("...");
			return;
		}
		
		enviando = true;
		$("#div_resultado_envio").html("Enviando/Sending...");
		$.ajax({
			type: 'POST',
			url: "enviarReporteDescarga.jsp",
			success: function(result){
	        	$("#div_resultado_envio").html(result);
	        	enviando = false;
	        }
        });
	}
				...
				<div id="main-inner" class="section-wrapper">
					<div class="section">
						<div class="section-inner clearfix fondoformulario">
							...
							<h2 class="titulo_naranja"><%=msj.getString("titulo_consulta_descarga_imagenes") %></h2>

							<div id="content">
								<div class="content-inner">
				
									<div id="block-consulta-reportes" class="block-gray block">
									<form id="generarReporteDescarga" name="generarGeporteDescarga" action="/MonitoreoBC-WEB/generarReporteDescargasServlet" method="post" onsubmit="validarRepDatasets()">
...
<%-- 															<input id="fFinal" name="fFinal" onclick="return noKeyData(event);" title="<%=msj.getString("descargas.fechaFinalTitle")%>" value=""> --%>
														...
														<input id="btnEnviar" name="btnEnviar" type="button" onclick="javascript:enviar();" value="<%=msj.getString("descargas.enviar")%>" class="btn btn-default" title="<%=msj.getString("descargas.enviar")%>">
														<input
													...
													<div id="div_resultado_envio"></div>
