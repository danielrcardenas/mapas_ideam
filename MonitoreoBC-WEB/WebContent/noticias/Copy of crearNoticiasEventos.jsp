...
<script>
	...
	function ModificarNotEv(idTab,consecutivo){
		document.getElementById('divCrear').style.display='none';
		document.getElementById('divCrear').style.border='hidden';
		document.getElementById('divModificar').style.display='none';
		document.getElementById('divModificar').style.border='hidden';
		...
		document.getElementById(idTab).style.display='block';
		alert('ModificarNotEv: '+consecutivo);
	}
		...
		if (r == true) {
			...
			$.ajax({
				...
				success: function(data, textStatus, jqXHR){
					...
					if (respuesta == "1") {
			        	alert("Noticia/Evento eliminado con éxito");
					}
					else {
						alert("Noticia/Evento NO eliminado... consecutivo: " + id_consecutivo);
					}
		        	eliminando = false;
		        },
		    	error: function (jqXHR, result, errorThrown) {
		    		alert("No fue posible eliminar el reporte: " + jqXHR.responseText);
		        	eliminando = false;
		        }
		    });
		}
						...
						<div class="section-inner clearfix fondoformulario">
							...
							<div id="content">
								<div class="content-inner">
									...
									<table id="block-consulta-reportes" >
									 <tr>
									 <td>
									
									    <div class="menu_noticias_eventos" style="text-align: center; display: block; margin-left: 20px">
									    <u class="u_noticias_eventos">
									        <li><a id="btnEliminar" style="cursor: pointer;" onclick="seleccionarTab('divEliminar',this);"><%=msj.getString("crearnoticiaseventos.37") %></a> </li>
											<li><a id="btnCrear"  style="cursor: pointer; " onclick="seleccionarTab('divCrear',this);"><%=msj.getString("crearnoticiaseventos.35") %></a> </li>
                                     ...
                                     <td>
										...
										<div id="divEliminar" style="display: none;" class="div_noticias_eventos">
										 <div class="block-gray block" style="overflow-x:auto;">		
											<h2><%=msj.getString("crearnoticiaseventos.39") %></h2>
											<form method="POST" enctype="multipart/form-data" action="/MonitoreoBC-WEB/eliminarNoticiasServlet" 
													id="formElmAdmNot" name="formElmAdmNot" onsubmit="return validarDatosForm()">
										...
										<div id="divCrear" class="div_noticias_eventos">
										...
										<div id="divModificar" style="display: none;" class="div_noticias_eventos">
											<div class="block-gray block">									
												<h2><%=msj.getString("crearnoticiaseventos.40") %></h2>
