...
<%@page import="co.gov.ideamredd.web.parametrizacion.dao.AlmacenaParametro"%>
...
<title>Almacenar Parámetro</title>
</head>
<%
...
<body>
<%
String respuesta = "Faltan privilegios/Lacking privileges.";
if(usuario != null) {
	if(ControlPermisos.tienePermiso(diccionarioPermisos, 2000)) {
		String PRTR_CONSECUTIVO = "";
		String PRTR_RUTA = "";

		/*
		Map<String, String[]> parameters = request.getParameterMap();
		for(String parameter : parameters.keySet()) {
		    if(parameter.startsWith("PRTR_CONSECUTIVO")) {
		        PRTR_CONSECUTIVO = parameters.get(parameter);
		    }
		    if(parameter.startsWith("PRTR_RUTA")) {
		        PRTR_RUTA = parameters.get(parameter);
		    }
		}
		*/
		
		PRTR_CONSECUTIVO = request.getParameter("PRTR_CONSECUTIVO");
		PRTR_RUTA = request.getParameter("PRTR_RUTA");
		
		if (!PRTR_CONSECUTIVO.equals("") && !PRTR_RUTA.equals("")) {
			respuesta = AlmacenaParametro.almacenarParametro(Integer.parseInt(PRTR_CONSECUTIVO), PRTR_RUTA);
		}
	}
}
out.print(respuesta);
