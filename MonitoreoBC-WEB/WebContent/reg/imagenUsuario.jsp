...
<script>
	...
	function validarDatosForm(){
		...
		var autorImagen=document.getElementById("autorImagen").value;
		...
		nombreImagen=nombreImagen.replace(/á/g, "&aacute;")
		.replace(/é/g, "&eacute;").replace(/í/g, "&iacute;")
		.replace(/ó/g, "&oacute;").replace(/ú/g, "&uacute;")
		.replace(/ñ/g, "&ntilde;");
		
		autorImagen=autorImagen.replace(/á/g, "&aacute;")
		.replace(/é/g, "&eacute;").replace(/í/g, "&iacute;")
		.replace(/ó/g, "&oacute;").replace(/ú/g, "&uacute;")
		.replace(/ñ/g, "&ntilde;");
		...
		if(nombreImagen == ""){resultado=false;mensaje=mensaje+"-Nombre de la imagen\n";}
		if(tipoLicencia == "-1"){resultado=false;mensaje=mensaje+"-Licencia\n";}
		if(descripcion == ""){resultado=false; mensaje=mensaje+"-Descripcion\n";}
		if(imagenUsuario == ""){resultado=false; mensaje=mensaje+"-Imagen\n";}
		if(imagenUsuario != ""){
			imagenUsuario=imagenUsuario.toLowerCase();
			if(imagenUsuario.indexOf(".png")==-1 && imagenUsuario.indexOf(".jpg")==-1)
			{
				resultado=false;
				mensaje=mensaje+"-Imagen(Formato incorrecto debe ser png o jpg)\n";
			}
		}
		...
		if (!resultado) {
			...
			document.getElementById("nombreImagen").value=nombreImagen;
			document.getElementById("descripcion").value=descripcion;
			document.getElementById("autorImagen").value=autorImagen;
		}
							...
							<div id="content">
								<div class="content-inner">
									...
									<div id="block-consulta-reportes" class="block-gray block">
										...
										<form method="POST" enctype="multipart/form-data" action="/MonitoreoBC-WEB/guardarImagenUsuarioServlet" 
										...
										<label for="exampleInputEmail1">Nombre de la imagen:<span class="obligatorio">*</span></label>
										<input type="text" name="nombreImagen" id="nombreImagen" style="width: 280px">
										<br><br>
										
										<label for="exampleInputEmail1">Licencia:<span class="obligatorio">*</span></label>
										<div class="select-wrapper" style="width: 300px">
											<select class="form-control" name="tipoLicencia" id="tipoLicencia">
												<option value="-1">Seleccionar</option>
												<option value="1">Publicar y descargar</option>
												<option value="2">Solo publicar</option>
											</select>
										</div>
										<br>
										
										<label for="exampleInputEmail1">Descripcion:<span class="obligatorio">*</span></label>
										<textarea style="resize: none; font-size: 15px; width: 280px; height: 100px;" name="descripcion" id="descripcion"></textarea>
										<br><br>
										
										<label for="exampleInputEmail1">Autor:</label>
										<input type="text" name="autorImagen" id="autorImagen" style="width: 280px">
										<br><br>
										
										<label for="exampleInputEmail1">Localizacion (Decimales):</label>
										<input type="text" name="latitud" id="latitud" style="width: 130px" placeholder="latitud" onkeypress="return valideValDec(event)">
										<input type="text" name="longitud" id="longitud" style="width: 130px" placeholder="longitud" onkeypress="return valideValDec(event)">
										<br><br>
										
										<label for="exampleInputEmail1">Adjunte la imagen(JPG, PNG):<span class="obligatorio">*</span></label>
										<input type="file" name="imagenUsuario" id="imagenUsuario" class="btn btn-default">
