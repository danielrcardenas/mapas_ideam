...
function setExtent() {
	if (window.parent.document.getElementById('cx')) {
		window.parent.document.getElementById('cx').value = mapa_wms.getCenter().lng;
	}

	if (window.parent.document.getElementById('cy')) {
		window.parent.document.getElementById('cy').value = mapa_wms.getCenter().lat;
	}

	if (window.parent.document.getElementById('zoom')) {
		window.parent.document.getElementById('zoom').value = mapa_wms.getZoom();
	}
}

mapa_wms.on('zoomend', function() {
	setExtent();
});

mapa_wms.on('moveend', function() {
	setExtent();
});

mapa_wms.on('dragend', function() {
	setExtent();
});

//wms_pixeles_srs = 'EPSG:3116';
...
// WMS Geoproceso
var wms = L.tileLayer.wms(wms_pixeles_url, {
    layers: '<%=identImagen%>',
    format: 'image/png',
    srs: wms_pixeles_srs,
    transparent: true,
