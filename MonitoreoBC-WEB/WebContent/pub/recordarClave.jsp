...
if(usuario !=null) {
	...
	String errorRegistro, errorRol, errorUsuarioIDEAM;
	try {
	...
	try {
		errorUsuarioIDEAM = (String) session.getAttribute("errorUsuarioIDEAM");
		session.setAttribute("errorUsuarioIDEAM", "No");
	} catch (Exception e) {
		errorUsuarioIDEAM = "No";
	}
...
alert("<%=msj.getString("recordarClave.alert.noExiste")%>"); 
<%}%>

<%if (errorRol == "Yes") {%>
alert("<%=msj.getString("recordarClave.alert.noRolPub")%>");
<%}%>	 

<%if (errorUsuarioIDEAM == "Yes") {%>
alert("<%=msj.getString("recordarClave.alert.noUsuarioIDEAM")%>");
<%}%>	 
	...
	function validar() {
		...
		if (document.getElementById("email").value == "") {
			mensaje = '<%=msj.getString("recordarClave.alert.campoVacio")%>';
			passed = false;
		} else if (!valideMail(document.getElementById("email").value)) {
			mensaje = "<%=msj.getString("recordarClave.alert.errorEmail")%>" ;
			passed = false;
		}
		if (!passed) {
			alert(mensaje);
							...
							<div id="content">
								<div class="content-inner">
									...
									<div id="block-consulta-reportes" class="block-gray block">
										<h2><%=msj.getString("recordarClave.RecordarClave")%></h2>
										<label for="exampleInputEmail1" style="margin-left: 20px">
											<%=msj.getString("recordarClave.CorreoElectronico")%>:
										</label> <input class="select-wrapper" name="auxEmail" id="auxEmail"
											size="30" style="width: 250px; margin-left: 20px">
										<div class="form-actions">
											<input type="button"
												value="<%=msj.getString("recordarClave.Recordar")%>"
												class="btn btn-default btn-ir" onclick="enviarFormRC()">
											<input type="button"
