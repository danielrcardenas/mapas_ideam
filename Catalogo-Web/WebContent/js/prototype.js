...
var Class = (function() {
	...
	function addMethods(source) {
		var ancestor = this.superclass && this.superclass.prototype, properties = Object
				.keys(source);
	...
	function Str(key, holder, stack) {
		...
		case 'object':
			...
			for ( var i = 0, length = stack.length; i < length; i++) {
				if (stack[i] === value) {
					throw new TypeError("Cyclic reference to '" + value
							+ "' in object");
				}
			}
			stack.push(value);
		...
		.extend(
				...
				(function() {
					...
					function truncate(length, truncation) {
						length = length || 30;
						truncation = Object.isUndefined(truncation) ? '...'
								: truncation;
										...
										function(hash, pair) {
											if ((pair = pair.split('='))[0]) {
												var key = decodeURIComponent(pair
														.shift()), value = pair.length > 1 ? pair
														.join('=')
														: pair[0];
					...
					function unfilterJSON(filter) {
						return this.replace(filter || Prototype.JSONFilter,
								'$1');
					}
					...
					return {
						...
						isJSON : isJSON,
						evalJSON : NATIVE_JSON_PARSE_SUPPORT ? parseJSON
								: evalJSON,
						include : include,
								...
								function(match) {
									...
									while (match != null) {
										...
										if (null == ctx || '' == match[3])
											break;
										expr = expr
												.substring('[' == match[3] ? match[1].length
														: match[0].length);
										match = pattern.exec(expr);
									}
...
var Enumerable = (function() {
	...
	function partition(iterator, context) {
		...
		var trues = [], falses = [];
		this.each(function(value, index) {
			(iterator.call(context, value, index, this) ? trues : falses)
					.push(value);
		}, this);
		return [ trues, falses ];
	}
...
var Hash = Class.create(Enumerable, (function() {
	function initialize(object) {
		this._object = Object.isHash(object) ? object.toObject() : Object
				.clone(object);
	}
		...
		.create(
				Ajax.Base,
				{
					...
					request : function(url) {
						...
						var params = Object.isString(this.options.parameters) ? this.options.parameters
						...
						try {
							...
							/*
							 * Force Firefox to handle ready state 4 for
					...
					setRequestHeaders : function() {
						...
						if (this.method == 'post') {
							...
							/*
							 * Force "Connection: close" for older Mozilla
							 * browsers to work around a bug where
							 * XMLHttpRequest sends an incorrect Content-length
						...
						for ( var name in headers)
							this.transport
									.setRequestHeader(name, headers[name]);
					},
...
Ajax.Response = Class
		.create({
			initialize : function(request) {
				...
				if ((readyState > 2 && !Prototype.Browser.IE)
						|| readyState == 4) {
					this.status = this.getStatus();
					this.statusText = this.getStatusText();
					this.responseText = String
							.interpret(transport.responseText);
		...
		.create(
				Ajax.Base,
				{
					...
					stop : function() {
						...
						(this.onComplete || Prototype.emptyFunction).apply(
								this, arguments);
					},
...
(function(GLOBAL) {
	...
	function Element(tagName, attributes) {
		...
		if (!ELEMENT_CACHE[tagName])
			ELEMENT_CACHE[tagName] = Element.extend(document
					.createElement(tagName));

		var node = shouldUseCreationCache(tagName, attributes) ? ELEMENT_CACHE[tagName]
				.cloneNode(false)
	...
	var SCRIPT_ELEMENT_REJECTS_TEXTNODE_APPENDING = (function() {
		...
		try {
			...
			isBuggy = !s.firstChild || s.firstChild
	...
	function update(element, content) {
		...
		if (ANY_INNERHTML_BUGGY) {
			if (tagName in INSERTION_TRANSLATIONS.tags) {
				...
				var nodes = getContentFromAnonymousElement(tagName, content
						.stripScripts());
				for ( var i = 0, node; node = nodes[i]; i++)
					element.appendChild(node);

			} else if (LINK_ELEMENT_INNERHTML_BUGGY && Object.isString(content)
	...
	function replace_IE(element, content) {
		...
		if (tagName in INSERTION_TRANSLATIONS.tags) {
			...
			var fragments = getContentFromAnonymousElement(tagName, content
					.stripScripts());
	...
	function insertContentAt(element, content, position) {
		...
		content = Object.toHTML(content);
		var tagName = ((position === 'before' || position === 'after') ? element.parentNode
		...
		for ( var property in styles) {
			if (property === 'opacity') {
				...
				if (property === 'float' || property === 'cssFloat') {
					property = Object.isUndefined(elementStyle.styleFloat) ? 'cssFloat'
							: 'styleFloat';
				}
	...
	function setOpacity_IE(element, value) {
		...
		style.filter = stripAlphaFromFilter_IE(filter) + 'alpha(opacity='
				+ (value * 100) + ')';
	...
	function checkElementPrototypeDeficiency(tagName) {
		...
		if (proto) {
			var id = '_' + (Math.random() + '').slice(2), el = document
					.createElement(tagName);
			proto[id] = 'x';
			var isBuggy = (el[id] !== 'x');
			delete proto[id];
	...
	function extend(element) {
		...
		var methods = Object.clone(Methods), tagName = element.tagName
				.toUpperCase();
	...
	function findDOMClass(tagName) {
		...
		var element = document.createElement(tagName), proto = element['__proto__']
...
(function() {
	...
	function getPixelValue(value, property, context) {
		...
		if (element && isPercentage) {
			...
			var isHorizontal = property.include('left')
					|| property.include('right') || property.include('width');

			var isVertical = property.include('top')
					|| property.include('bottom') || property.include('height');
					...
					Hash,
					{
						initialize : function($super, element, preCompute) {
							...
							if (preCompute) {
								...
								this._begin();
								Element.Layout.PROPERTIES.each(this._compute,
										this);
								this._end();
						...
						_set : function(property, value) {
							return Hash.prototype.set.call(this, property,
									value);
						},
						...
						get : function($super, property) {
							...
							return value === null ? this._compute(property)
									: value;
						},
						...
						_begin : function() {
							...
							element.store('prototype_original_styles',
									originalStyles);
							...
							if (width && (positionedWidth === width)) {
								...
								var parent = element.parentNode, pLayout = $(
										parent).getLayout();
						...
						_compute : function(property) {
							...
							return this._set(property, COMPUTATIONS[property]
									.call(this, this.element));
						},

						_isPrepared : function() {
							return this.element.retrieve(
									'prototype_element_layout_prepared', false);
						},

						_setPrepared : function(bool) {
							return this.element.store(
									'prototype_element_layout_prepared', bool);
						},
						...
						toCSS : function() {
							...
							keys.each(function(key) {
								if (!Element.Layout.PROPERTIES.include(key))
									return;
								if (Element.Layout.COMPOSITE_PROPERTIES
										.include(key))
									return;
					...
					Element.Layout,
					{
						...
						COMPUTATIONS : {
							'height' : function(element) {
								...
								var bTop = this.get('border-top'), bBottom = this
										.get('border-bottom');

								var pTop = this.get('padding-top'), pBottom = this
										.get('padding-bottom');

								if (!this._preComputing)
									this._end();

								return bHeight - bTop - bBottom - pTop
										- pBottom;
							},
							...
							'width' : function(element) {
								...
								var bLeft = this.get('border-left'), bRight = this
										.get('border-right');

								var pLeft = this.get('padding-left'), pRight = this
										.get('padding-right');
							...
							'padding-box-height' : function(element) {
								var height = this.get('height'), pTop = this
										.get('padding-top'), pBottom = this
										.get('padding-bottom');
							...
							'padding-box-width' : function(element) {
								var width = this.get('width'), pLeft = this
										.get('padding-left'), pRight = this
										.get('padding-right');
							...
							'margin-box-height' : function(element) {
								var bHeight = this.get('border-box-height'), mTop = this
										.get('margin-top'), mBottom = this
										.get('margin-bottom');
							...
							'margin-box-width' : function(element) {
								var bWidth = this.get('border-box-width'), mLeft = this
										.get('margin-left'), mRight = this
										.get('margin-right');
							...
							'bottom' : function(element) {
								var offset = element.positionedOffset(), parent = element
										.getOffsetParent(), pHeight = parent
										.measure('height');
	...
	if ('getBoundingClientRect' in document.documentElement) {
		Object.extend(Element.Layout.COMPUTATIONS, {
			'right' : function(element) {
				...
				var rect = element.getBoundingClientRect(), pRect = parent
	...
	Element.Offset = Class.create({
		...
		relativeTo : function(offset) {
			return new Element.Offset(this.left - offset.left, this.top
					- offset.top);
		},

		inspect : function() {
			return "#<Element.Offset left: #{left} top: #{top}>"
					.interpolate(this);
		},
	...
	function absolutize(element) {
		...
		var eOffset = element.viewportOffset(), pOffset = offsetParent
				.viewportOffset();
	...
	function undoPositioned(element) {
		element = $(element);
		var storage = Element.getStorage(element), madePositioned = storage
	...
	function makeClipping(element) {
		...
		var storage = Element.getStorage(element), madeClipping = storage
	...
	function undoClipping(element) {
		element = $(element);
		var storage = Element.getStorage(element), overflow = storage
				.get('prototype_made_clipping');
	...
	var Expr = Sizzle.selectors = {
		...
		filter : {
			...
			ATTR : function(elem, match) {
				var name = match[1], result = Expr.attrHandle[name] ? Expr.attrHandle[name]
						(elem)
				...
				if (!seed && !Sizzle.isXML(context)) {
					...
					if (match
						...
						if (match[1]) {
							...
							return makeArray(context
									.getElementsByClassName(match[2]), extra);
						}
					}
	...
	(function() {
		...
		if (matches) {
			var disconnectedMatch = !matches.call(
					document.createElement("div"), "div"), pseudoWorks = false;
...
Form.Element.Methods = {
	...
	activate : function(element) {
		...
		try {
			element.focus();
			if (element.select
					&& (element.tagName.toLowerCase() != 'input' || !(/^(?:button|reset|submit)$/i
							.test(element.type))))
				element.select();
		} catch (e) {
		}
...
Form.Element.Serializers = (function() {
	...
	function select(element, value) {
		if (Object.isUndefined(value))
			return (element.type === 'select-one' ? selectOne : selectMany)
					(element);
				...
				PeriodicalExecuter,
				{
					...
					execute : function() {
						...
						if (Object.isString(this.lastValue)
								&& Object.isString(value) ? this.lastValue != value
...
Abstract.EventObserver = Class
		.create({
			...
			registerFormCallbacks : function() {
				Form.getElements(this.element)
						.each(this.registerCallback, this);
			},
			...
			registerCallback : function(element) {
				if (element.type) {
					...
					case 'checkbox':
					case 'radio':
						Event.observe(element, 'click', this.onElementEvent
								.bind(this));
						break;
					default:
						Event.observe(element, 'change', this.onElementEvent
								.bind(this));
						break;
					}
				}
...
(function(GLOBAL) {
	...
	function _isButtonForDOMEvents(event, code) {
		return event.which ? (event.which === code + 1)
				: (event.button === code);
	}
		...
		if (!window.addEventListener) {
			...
			_isButton = function(event, code) {
				return isIELegacyEvent(event) ? _isButtonForLegacyEvents(event,
						code) : _isButtonForDOMEvents(event, code);
			}
		}
	} else if (Prototype.Browser.WebKit) {
	...
	function _element(event) {
		...
		if (currentTarget && currentTarget.tagName) {
			if (type === 'load'
					|| type === 'error'
					|| (type === 'click'
							&& currentTarget.tagName.toLowerCase() === 'input' && currentTarget.type === 'radio'))
				node = currentTarget;
		}
	...
	function stopObserving(element, eventName, handler) {
		element = $(element);
		var handlerGiven = !Object.isUndefined(handler), eventNameGiven = !Object
	...
	function stopObservingElement(element) {
		var uid = getUniqueElementID(element), registry = getRegistryForElement(
				element, uid);
	...
	function on(element, eventName, selector, callback) {
		...
		return new Event.Handler(element, eventName, selector, callback)
				.start();
	}
...
if (!document.getElementsByClassName)
	document.getElementsByClassName = function(instanceMethods) {
		function iter(name) {
			return name.blank() ? null
					: "[contains(concat(' ', @class, ' '), ' " + name + " ')]";
		}

		instanceMethods.getElementsByClassName = Prototype.BrowserFeatures.XPath ? function(
				element, className) {
			...
			return cond ? document._getElementsByXPath('.//*' + cond, element)
				...
				: function(element, className) {
					...
					for ( var i = 0, child, cn; child = nodes[i]; i++) {
						if (child.className
								&& (cn = ' ' + child.className + ' ')
								&& (cn.include(className) || (classNames && classNames
										.all(function(name) {
											return !name.toString().blank()
													&& cn.include(' ' + name
															+ ' ');
										}))))
