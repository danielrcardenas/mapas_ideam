...
//import co.gov.ideamredd.parametros.servlets.EditarParametroServlet;
...
public class ActualizarParametro {
	...
	public boolean modificarParametro() {
		...
		try {
			log.info(ActualizarParametro.class+" Inicio actualizar parametros");
	 ...
	 */
	 public String getRutaProviders(){
		 String rutaProviders = "";
			try {
				...
				log.error(ActualizarParametro.class+ "[modificarParametros] Fallo, no se puede obtener la ruta PROVIDERS de la tabpa RED_PARAMETROS",e);
				e.printStackTrace();
	 ...
	 public void escribirArchivo(String rutaProviders) {
		 try {
			 ...
			 log.error(ActualizarParametro.class+"[modificarParametros] Fallo, no se puede modificar la ruta de thumnails, verifique que el archivo Providers.fac existe en la tabla RED_PARAMETROS y que el archivo tenga permisos de escritura",e);
	 ...
	 public boolean editarRutaMetadato(String rutaProviders, String nuevaRutaMetadato) {
		 ...
		 try {
			...
			log.error(ActualizarParametro.class+"[modificarParametros] Fallo, no se puede modificar la ruta de metadatos, verifique que el archivo Providers.fac existe en la tabla RED_PARAMETROS y que el archivo tenga permisos de lectura y escritura",e);
			e.printStackTrace();
			return false;
		} 
		 resultado=true;
		 ...
		 try {
			fileToRead = new Scanner(new File(rutaProviders));
			 for (String line; fileToRead.hasNextLine() && (line = fileToRead.nextLine()) != null; ) {
				 ...
				 fileToRead.close();
				 resultado=true;
				 log.info(ActualizarParametro.class+"[modificarRutaThumbnail] Termino");
		} catch (FileNotFoundException e) {
			log.error(ActualizarParametro.class+"[modificarRutaThumbnail] Fallo, no se puede modificar la ruta de thumbnail, verifique que el archivo Providers.fac existe en la tabla RED_PARAMETROS y que el archivo tenga permisos de lectura y escritura",e);
			// TODO Auto-generated catch block
			e.printStackTrace();
			return false;
		} 
