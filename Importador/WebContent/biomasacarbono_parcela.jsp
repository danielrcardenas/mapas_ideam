...
if (Auxiliar.tieneAlgo(PRCL_CONSECUTIVO)) {
	CNPR_CONS_PARCELA = PRCL_CONSECUTIVO;
}
%>
...
function nueva(f) {
	url = 'Parcela?accion=biomasacarbono_parcela&control=&PRCL_CONSECUTIVO=<%= PRCL_CONSECUTIVO %>&usuario=<% out.print(request.getParameter("usuario"));%>&idioma=<% out.print(request.getParameter("idioma"));%>';
	location.href = url;
}
...
<form action='Parcela?accion=contactos_parcela&usuario=<% out.print(request.getParameter("usuario"));%>&idioma=<% out.print(request.getParameter("idioma"));%>' method='post'>
...
<th class="etiqueta">
<div><label>
<% try { out.print(msj.getString("CNPR_CONS_PARCELA")); } catch (MissingResourceException e) { out.print("Consecutivo Parcela Proyecto REDD" + ".."); } %>
</label></div>
</th>
<td>
...
<input type="button" value="Buscar" onclick="ajax_opciones('Parcela','CNPR_CONS_CONTACTO','div_ajax_contactos','?accion=encontrar_contactos&USR_CONS_TIPOIDENT='+this.form.USR_CONS_TIPOIDENT.value+'&USR_ID='+this.form.USR_ID.value,'','No se encontraron contactos.');" />
</div>
...
String USUARIO = Auxiliar.nzObjStr(request.getAttribute("USUARIO"), "").toString();
%>

<select class="form-control" name="CNPR_CONS_CONTACTO" id="CNPR_CONS_CONTACTO" size="4" onclick="javascript:this.form.INDV_ESPECIE.value=this.value;">
<option selected value="<%= CNPR_CONS_CONTACTO %>"><%= USUARIO %></option>
</select>
...
<div class="tabla_contactos" id="div_tabla_contactos">

<% 
out.println(request.getAttribute("tabla_contactos")); 
%>
