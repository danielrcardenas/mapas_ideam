...
<title>
<% try { out.print(msj.getString(yo+"Imagenes_de_Parcela")); } catch (MissingResourceException e) { out.print("Imágenes de la Parcela" + ".."); } %>
</title>
...
String solovisor = Auxiliar.nzObjStr(request.getAttribute("solovisor"), "").toString();
%>
...
String str_fotos = Auxiliar.nzObjStr(request.getAttribute("PRCL_NOMBREARCHIVO"), "").toString();
String [] a_fotos = str_fotos.split(",");

String primera_foto = "";

if (a_fotos.length > 0) {
	primera_foto = a_fotos[0];
}
else {
	primera_foto = "";
}
...
<form name="f" action='ImagenesParcela' method='post' enctype='multipart/form-data'>

<%
if (!solovisor.equals("1")) {
%> 
<div class="campo" >
<div class="etiqueta">
<% 
try { out.print(msj.getString(yo+"Imagen_parcela_a_subir")); } catch (MissingResourceException e) { out.print("Imagen a subir (sólo formatos .png o .jpg)" + ".."); } 
%> 
</div>
...
<input type="hidden" name="PRCL_CONSECUTIVO" id="PRCL_CONSECUTIVO" value="<%= PRCL_CONSECUTIVO %>" />
...
<input type="hidden" name="f_imagen" />
...
String carpeta = dbREDD.obtenerDato("SELECT PRTR_RUTA FROM RED_PARAMETRO WHERE PRTR_NOMBRE='carpeta_imagenes_parcelas'", "imagenes_parcelas");
%>

<div id="fp_gallery" class="fp_gallery">
<img src='<% out.print(carpeta + "/" + PRCL_CONSECUTIVO + "/" + primera_foto); %>' alt="" class="fp_preview" style="display:none;" />
...
if (a_fotos.length > 0) {
	...
	for (i=0; i<a_fotos.length; i++) {
		if (!a_fotos[i].equals("")) {
			out.print("<div class='content'><div><a href='#'><img class='imagen' src='" + carpeta + "/" + PRCL_CONSECUTIVO + "/" + a_fotos[i] + "' alt='" + carpeta + "/" + PRCL_CONSECUTIVO + "/" + a_fotos[i] + "' class='thumb' /></a></div></div>");
		}
	}
}
%>
