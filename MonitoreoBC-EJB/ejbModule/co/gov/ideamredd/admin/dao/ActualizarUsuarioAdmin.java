...
import co.gov.ideamredd.admin.entities.Perfil;
...
@Stateless public class ActualizarUsuarioAdmin {

	@EJB
	ConexionBD					conexion;
	@EJB
	ConsultaUsuarioAdmin		consUs;
	...
	/**
	 * Método usado para modificar los datos de un usuario, el id del usuario debe ser definido antes con el metodo set, respectivo.
	 */
	public boolean modificarUsuario() {
		boolean resultado = false;
		try {
			log = SMBC_Log.Log(ActualizarUsuarioAdmin.class);
			conn = conexion.establecerConexion();
			CallableStatement stmt = conn.prepareCall("{call RED_PK_USUARIOS.Usuario_Actualiza(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			stmt.setInt("una_Llave", idUsuario);
			stmt.setString("un_login", un_login);
			stmt.setString("un_Nombre", un_Nombre);
			stmt.setString("un_ApellidoUno", null);
			stmt.setString("un_ApellidoDos", null);
			stmt.setInt("un_TipoIdentificacion", un_TipoIdentificacion);
			stmt.setString("una_Identificacion", una_Identificacion);
			stmt.setInt("un_TipoPersona", un_TipoPersona);
			stmt.setInt("un_Pais", un_Pais);
			stmt.setString("una_Direccion", una_Direccion);
			stmt.setString("un_TelefonoOficina", un_TelefonoOficina);
			stmt.setString("un_Celular", un_Celular);
			stmt.setString("un_CorreoElectronico", un_CorreoElectronico);
			if (!una_Clave.equals("")) {
				stmt.setString("una_Clave", Encript.getEncodedPassword(una_Clave));
			}
			else {
				stmt.setString("una_Clave", null);
			}
			stmt.setString("es_Activo", es_Activo);
			stmt.setDate("una_FechaCreacion", una_FechaCreacion);
			stmt.setInt("un_depto", un_Departamento);
			stmt.setInt("un_municipio", un_Municipio);
			stmt.setString("una_organizacion", null);
			stmt.setString("un_cargo", null);
			stmt.registerOutParameter("un_Resultado", OracleTypes.VARCHAR);
			stmt.registerOutParameter("una_Respuesta", OracleTypes.NUMBER);
			stmt.execute();
			System.out.println(stmt.getObject("un_Resultado"));

			if (Integer.valueOf(stmt.getInt("una_Respuesta")) == 1) {
				if (!licencia.isEmpty()) {
					insertarLicenciasPersona(idUsuario);
				}
				Usuario auxUs = consUs.consultarUsuarioPorLogin(un_login);
	...
	/**
	 * Método usado para modificar los datos de un usuario de apollo.
	 */
	...
	/**
	 * Método usado para modificar el rol de un usuario.
	 */
	...
	/**
	 * Método usado para modificar la contrase�a de un usuario.
	 */
	public void modificarClaveUsuario(String clavePlainText, int document) {
		try {
			log = SMBC_Log.Log(ActualizarUsuarioAdmin.class);
			conn = conexion.establecerConexion();
			...
			actualizarClave = conn.prepareCall("{call RED_PK_APOLLO.Usuario_Actualiza_Clave(?,?,?)}");
	...
	/**
	 * Método usado para insertar actividades, relacionadas con un usuario, debe definirse antes lo ids.
	 */
	...
	/**
	 * Método usado para relacionar roles con un usuario.
	 */
	...
	/**
	 * Método usado para relacionar licencias con un usuario.
	 */
	...
	/**
	 * Método usado para activar un usuario.
	 */
	...
	/**
	 * Método usado para desactivar un usuario.
	 */
