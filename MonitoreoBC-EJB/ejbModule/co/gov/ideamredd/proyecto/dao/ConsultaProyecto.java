...
import co.gov.ideamredd.proyecto.entities.CAR;
import co.gov.ideamredd.proyecto.entities.ConteoProyecto;
import co.gov.ideamredd.proyecto.entities.Depto;
import co.gov.ideamredd.proyecto.entities.DocumentosAsociados;
import co.gov.ideamredd.proyecto.entities.Metodologia;
import co.gov.ideamredd.proyecto.entities.Municipios;
import co.gov.ideamredd.proyecto.entities.Pais;
import co.gov.ideamredd.proyecto.entities.Proyecto;
import co.gov.ideamredd.proyecto.entities.Tenencia;
import co.gov.ideamredd.proyecto.entities.TipoBosque;
import co.gov.ideamredd.util.Util;

/**
 * clase usuada para cargar datos de proyectos consultados en la plataforma.
 */
public class ConsultaProyecto {
	...
	public static Proyecto consultarProyectoId(Integer idProyecto) {
		...
		try {
			...
			CallableStatement consultaProyecto = conn.prepareCall("{call RED_PK_PROYECTOS.consultarProyectoId(?,?)}");
			consultaProyecto.setInt("un_proyecto", idProyecto);
			consultaProyecto.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultaProyecto.execute();
			ResultSet r = (ResultSet) consultaProyecto.getObject("un_Resultado");
			while (r.next()) {
	...
	public static Depto consultarDepto(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaDepto = conn.prepareCall("{call RED_PK_PROYECTOS.DeptoProyecto_Consulta(?,?)}");
			consultaDepto.setInt("un_Proyecto", idProyecto);
			consultaDepto.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultaDepto.execute();
			ResultSet r = (ResultSet) consultaDepto.getObject("un_Resultado");
			while (r.next()) {
	...
	public static Municipios consultarMcipio(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaMunicipio = conn.prepareCall("{call RED_PK_PROYECTOS.MunicipioProyecto_Consulta(?,?)}");
	...
	public static CAR consultarCAR(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaCar = conn.prepareCall("{call RED_PK_PROYECTOS.CarProyecto_Consulta(?,?)}");
	...
	public static Pais consultarPais(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaPais = conn.prepareCall("{call RED_PK_PROYECTOS.PaisProyecto_Consulta(?,?)}");
	...
	public static TipoBosque consultarTipoBosque(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaBosque = conn.prepareCall("{call RED_PK_PROYECTOS.tipoBosqueProyecto_Consulta(?,?)}");
			consultaBosque.setInt("un_proyecto", idProyecto);
			consultaBosque.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultaBosque.execute();
			ResultSet r = (ResultSet) consultaBosque.getObject("un_Resultado");
			while (r.next()) {
	...
	public static Metodologia consultarMetodologia(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultaMetologia = conn.prepareCall("{call RED_PK_PROYECTOS.MetodologiaProyecto_Consulta(?,?)}");
			consultaMetologia.setInt("un_Proyecto", idProyecto);
			consultaMetologia.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultaMetologia.execute();
			ResultSet r = (ResultSet) consultaMetologia.getObject("un_Resultado");
			while (r.next()) {
				metodologia.setMetodologiaId(r.getInt(1));
				metodologia.setMetodologiaNombre(r.getString(2));
				metodologia.setMetodologiaEcuacion(r.getString(3) == null ? "" : r.getString(3));
				metodologia.setMetodologiaDirArchivo(r.getString(4) == null ? "" : r.getString(4));
	...
	public static Tenencia consultarTenencia(Integer idProyecto) {
		...
		try {
			conn = dataSource.getConnection();
			CallableStatement consultarTenenciaProyecto = conn.prepareCall("{call RED_PK_PROYECTOS.TenenciaProyecto_Consulta(?,?)}");
			consultarTenenciaProyecto.setInt("un_Proyecto", idProyecto);
			consultarTenenciaProyecto.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultarTenenciaProyecto.execute();
			ResultSet resultSet = (ResultSet) consultarTenenciaProyecto.getObject("un_Resultado");
	...
	public static ArrayList<ActividadRedd> consultarActividadProyecto(Integer idProyecto) {
		ArrayList<ActividadRedd> actividades = new ArrayList<ActividadRedd>();
		try {
			conn = dataSource.getConnection();
			CallableStatement consultarActividadProyecto = conn.prepareCall("{call RED_PK_PROYECTOS.ActividadProyecto_Consulta(?,?)}");
			consultarActividadProyecto.setInt("un_Proyecto", idProyecto);
			consultarActividadProyecto.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultarActividadProyecto.execute();
			ResultSet resultSet = (ResultSet) consultarActividadProyecto.getObject("un_Resultado");
			while (resultSet.next()) {
				ActividadRedd actividadRedd = new ActividadRedd();
				actividadRedd.setConsecutivo(Integer.valueOf(resultSet.getObject(1).toString()));
	...
	public static String[] consultarGeoProyecto(Integer idProyecto) {
		// Clob geometria = null;
		ArrayList<String> coordAux = new ArrayList<String>();
		String[] coordenadas = null;
		try {
			conn = dataSource.getConnection();
			CallableStatement consultarPaisProyecto = conn.prepareCall("{call RED_PK_GEOMETRIA.Proyecto_Consulta(?,?)}");
			consultarPaisProyecto.setInt("un_Consecutivo", idProyecto);
			consultarPaisProyecto.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultarPaisProyecto.execute();
			// geometria = (Clob)
			// consultarPaisProyecto.getObject("un_Resultado");

			ResultSet resultSet = (ResultSet) consultarPaisProyecto.getObject("un_Resultado");
			while (resultSet.next()) {
				coordAux.add(resultSet.getString(1));
				coordAux.add(resultSet.getString(2));
			}
			...
			coordenadas[1] = "";
			for (int xy = 0; xy < coordAux.size(); xy++) {
				coordenadas[1] = coordenadas[1] + coordAux.get(xy).toString(); // Util.obtenerDatosGeometria(Util.clobStringConversion(geometria));
				if (xy + 1 < coordAux.size()) {
	...
	public static ArrayList<DocumentosAsociados> consultarDocumentosProyecto(Integer idProyecto) {
		ArrayList<DocumentosAsociados> documentos = new ArrayList<DocumentosAsociados>();
		try {
			conn = dataSource.getConnection();
			CallableStatement consultarDocumentosProyecto = conn.prepareCall("{call RED_PK_PROYECTOS.DocumentoAsociado_Consultar(?,?)}");
			consultarDocumentosProyecto.setInt("un_Proyecto", idProyecto);
			consultarDocumentosProyecto.registerOutParameter("un_Resultado", OracleTypes.CURSOR);
			consultarDocumentosProyecto.execute();
			ResultSet resultSet = (ResultSet) consultarDocumentosProyecto.getObject("un_Resultado");
			...
			while (resultSet.next()) {
				...
				documentosAsociados.setUna_Ruta(resultSet.getString(3));
	...
	public static ArrayList<ConteoProyecto> contarProyectos(String tipo, String agregacion, String desde, String hasta) {
		ArrayList<ConteoProyecto> conteoProyectos = new ArrayList<ConteoProyecto>();
		try {
			conn = dataSource.getConnection();
			CallableStatement cstmt = conn.prepareCall("{call RED_PK_PROYECTOS.contarProyectos_"+tipo+"_"+agregacion+"(?,?,?,?)}");
			if (desde.length() == 10 ) {
				cstmt.setString("desde", desde);
			}
			else {
				cstmt.setNull("desde", OracleTypes.NULL);
			}
			if (hasta.length() == 10) {
				cstmt.setString("hasta", hasta);
			}
			else {
				cstmt.setNull("hasta", OracleTypes.NULL);
			}
			cstmt.registerOutParameter("rs", OracleTypes.CURSOR);
			cstmt.registerOutParameter("sqlquery", OracleTypes.VARCHAR);
			cstmt.execute();
			ResultSet rs = (ResultSet) cstmt.getObject("rs");
			while (rs.next()) {
				ConteoProyecto conteoProyecto = new ConteoProyecto();
				conteoProyecto.setConteo(rs.getInt("CONTEO"));
				if (!tipo.equals("NAL")) {
					conteoProyecto.setDimension1(rs.getString(2));
				}
				if (!agregacion.equals("TOTAL")) {
					conteoProyecto.setDimension2(rs.getString(agregacion));
				}
				conteoProyectos.add(conteoProyecto);
			}
			rs.close();
			cstmt.close();
			conn.close();
