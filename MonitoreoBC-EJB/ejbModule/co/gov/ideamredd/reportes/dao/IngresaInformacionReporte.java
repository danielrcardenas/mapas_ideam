...
public class IngresaInformacionReporte {
	...
	public Integer IngresarReporte(Reportes reporte) {
		...
		try {
			...
			insertarReporte.setString("un_identImagen", reporte.getIdentImagen());
			//insertarReporte.setInt("un_publicado", 0); //Por defecto el reporte no esta publicado, por lo tanto se asigna cero esta especificado en el procedimiento almacenado
			...
			log.info("[IngresarReporte] Termino Final");
			System.out.println("[IngresarReporte] En tabla REDD_REPORTES Termino Final");
			insertarReporte.close();
			conn.close();
		} catch (Exception e) {
			log.error("[IngresarReporte] Fallo");
			System.out.println("[IngresarReporte] En tabla REDD_REPORTES Fallo");
			e.printStackTrace();
			/*
	...
	public void IngresarInfoCobertura(InformacionReporteCobertura cobertura) {
		try {
			...
			//Las capas ya no tienen esta clase, solo se utilizaba andtes cuando se cruzaban las dos capas de bosque antes de cruzarla con la capa base. Ya no se reporta
			insertarInfoCobertura.setDouble("un_bosquepuno", 0 );
