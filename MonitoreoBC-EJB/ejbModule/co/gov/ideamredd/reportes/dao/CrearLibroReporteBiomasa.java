...
/**
 * Clase usada para generar el reporte xls de biomasa y carbono.
 */
public class CrearLibroReporteBiomasa {
	...
	public XSSFWorkbook construirLibroReporte(Integer divisionTerritorial, String[] periodo) {

		hoja = crearLibro();
		estilo = estiloTituloReporte(libro);
		crearEncabezado(periodo, divisionTerritorial);
	...
	private void crearEncabezado(String[] periodo, Integer divisionTerritorial) {

		XSSFRow fila1 = hoja.createRow(2);
		XSSFRow tituloPeriodo = hoja.createRow(1);
		indiceColumna = 1;
		inicio = 1;
		if (divisionTerritorial == 4) {
			for (int i = 0; i < periodo.length; i++) {
				crearTituloBC_Nacional(fila1, tituloPeriodo, periodo[i]);
			}
		}
		else if (divisionTerritorial == 3) {
			for (int i = 0; i < periodo.length; i++) {
				crearTituloBC_X_AREAHIDROGRAFICA(fila1, tituloPeriodo, periodo[i]);
			}
		}
		else if (divisionTerritorial == 5) {
			for (int i = 0; i < periodo.length; i++) {
				crearTituloBC_X_TIPOBOSQUE(fila1, tituloPeriodo, periodo[i]);
			}
		}
		else {
			for (int i = 0; i < periodo.length; i++) crearTitulo(fila1, tituloPeriodo, periodo[i]);
		}

	}

	private void crearTituloConsolidado(XSSFRow fila1, XSSFRow tituloPeriodo, String periodo) {

		hoja.setColumnWidth(0, 1000);
		XSSFCell zonaVida = fila1.createCell(indiceColumna);
		zonaVida.setCellValue(infoReportes.getString("reporte.biomasa.zonaVida"));
		...
		CellRangeAddress mergeTitulo = new CellRangeAddress(tituloPeriodo.getRowNum(), tituloPeriodo.getRowNum(), inicio, indiceColumna);
		hoja.addMergedRegion(mergeTitulo);
		indiceColumna++;
		inicio = indiceColumna;
	}

	private void crearTituloBC_Nacional(XSSFRow fila1, XSSFRow tituloPeriodo, String periodo) {
		...
		hoja.setColumnWidth(indiceColumna, 4000);
		indiceColumna++;
		XSSFCell nivelIncertidumbre = fila1.createCell(indiceColumna);
		nivelIncertidumbre.setCellValue(infoReportes.getString("reporte.biomasa.nivelIncertidumbre"));
		nivelIncertidumbre.setCellStyle(estilo);
		hoja.setColumnWidth(indiceColumna, 4000);
		XSSFCell per = tituloPeriodo.createCell(inicio);
		per.setCellValue(periodo);
		per.setCellStyle(estilo);
	...
	private void crearTitulo(XSSFRow fila1, XSSFRow tituloPeriodo, String periodo) {
		...
		areaHidro.setCellValue(infoReportes.getString("reporte.biomasa.areaHidrografica"));
	...
	private void crearTituloBC_X_AREAHIDROGRAFICA(XSSFRow fila1, XSSFRow tituloPeriodo, String periodo) {
	...
	private void crearTituloBC_X_TIPOBOSQUE(XSSFRow fila1, XSSFRow tituloPeriodo, String periodo) {
		
		hoja.setColumnWidth(0, 1000);
		XSSFCell areaHidro = fila1.createCell(indiceColumna);
		areaHidro.setCellValue(infoReportes.getString("reporte.biomasa.zonaVida"));
		areaHidro.setCellStyle(estilo);
		hoja.setColumnWidth(indiceColumna, 8000);
		indiceColumna++;
		XSSFCell area = fila1.createCell(indiceColumna);
