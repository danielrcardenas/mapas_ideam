...
/**
 * Clase usada para consulta de reportes de bosque , no bosque.
 */
...
public class ConsultarReporteBosqueNoBosque {
	...
	public void consultarReporte(Integer territorio, String[] periodo, Integer[] idReporte) {
		libroReporte = new CrearLibroReporteBNB();
		libro = libroReporte.construirLibroReporte(territorio, periodo);
		File reporte = new File(parametro.getParametro("ruta_reportes") + libroReporte.getNombreReporte() + ".xlsx");

		Integer indice = Integer.valueOf(infoReportes.getString("reporte.bnb.cantidadGraficos"));
		...
		if (territorio != 4) {
			...
			if (territorio == 1) {
				for (int i = 0; i < periodo.length; i++) {
					webDatosGraficas.add(new Double[Integer.valueOf(infoReportes.getString("cantidadCarSinPonderado"))][3][1]);
				}
			}
			if (territorio == 2) {
				for (int i = 0; i < periodo.length; i++) {
					webDatosGraficas.add(new Double[Integer.valueOf(infoReportes.getString("CantidadBosqueSinPonderado"))][3][1]);
				}
			}
			if (territorio == 3) {
				for (int i = 0; i < periodo.length; i++) {
					webDatosGraficas.add(new Double[Integer.valueOf(infoReportes.getString("numeroAreasHidrograficas"))][3][1]);
				}
			}

			for (int i = 0; i < periodo.length; i++) {
				for (int j = 0; j < indice; j++) {
					graficos.llenarDatosGrafWeb(periodo, idReporte[i], i, j, webDatosGraficas);
				}
			}
			...
			graficos.llenarDatosConsolidadoGrafWeb(porcentajes, periodo, webConsolidadoNacional);
		...
		if (!reporte.exists() || true) {
			if (territorio != 4) {
				...
				for (int i = 0; i < periodo.length; i++) {
					for (int j = 0; j < indice; j++) {
						data.add(graficos.createDataset(periodo, idReporte[i], j));
						dataPie.add(graficos.createPieDataset(periodo, idReporte[i], j));
				...
				for (int j = 0; j < data.size(); j++) {
					insertarImagen(graficos.createChartBarras(data.get(j), graf, null, periodosAux.get(j)), graf, columna);
					if (graf >= 3) {
						graf = 0;
						columna = columna + 13;
					}
				...
				for (int j = 0; j < dataPie.size(); j++) {
					insertarImagenPie(graficos.createChartTortas(dataPie.get(j), graf, periodosAux.get(j)), graf, columna);
					if (graf >= 3) {
						graf = 0;
						columna = columna + 15;
					}
				...
				if (idReporte.length > 1) {
					for (int j = 0; j < 3; j++) {
						dataTimeseries.add(graficos.createTimeseriesDataset(periodo, idReporte, j));
					}
					for (int j = 0; j < dataTimeseries.size(); j++) {
						insertarImagenTimeseries(graficos.createChartTimeseries(dataTimeseries.get(j), j), j);
					}
				}
				...
				for (int i = 0; i < periodo.length; i++) {
					...
					int j = 0;
					while (j < Integer.valueOf(infoReportes.getString("BosqueNoBosqueCategorias"))) {
						porcentajes.remove(0);
						j++;
					}
					insertarImagenConsolidado(graficos.createBarChartConsolidado(null, 0, dataB, periodo[i]), i, 2, 1);
				}
				for (int i = 0; i < periodo.length; i++) {
					dataP = graficos.createPieDatasetConsolidado(porcentajesPie);
					int j = 0;
					while (j < Integer.valueOf(infoReportes.getString("BosqueNoBosqueCategorias"))) {
						porcentajesPie.remove(0);
						j++;
					}
					insertarImagenConsolidado(graficos.createPieChartConsolidado(null, 0, dataP, periodo[i]), i, 2, 2);

				}
				if (periodo.length > 1) {
					dataT = graficos.createTimeSDatasetConsolidado(porcentajesTimeB, porcentajesTimeNB, porcentajesTimeSI, nombrePeriodos);
					insertarImagenConsolidado(graficos.createTimeSChartConsolidado(null, 0, dataT, ""), 0, 2, 3);
				}
			}
			escribirReportes(parametro.getParametro("ruta_reportes"), libroReporte.getNombreReporte(), libro);
		}
	...
	private void poblarReporteConsolidado(Integer[] idReporte, String[] periodo) {
		...
		for (int k = 0; k < periodo.length; k++) {
			...
			ArrayList<InformacionReporteBosque> info = car.consultarInfoBosque(idReporte[k]);
			...
			Double pBosque = Util.calcularPorcentaje(total.intValue(), totalBosque.intValue());
			pb.setCellValue(df.format(pBosque));
			pb.setCellStyle(estiloDivisionReporte(libro));
			Double pNoBosque = Util.calcularPorcentaje(total.intValue(), totalNoBosque.intValue());
			pnb.setCellStyle(estiloDivisionReporte(libro));
			pnb.setCellValue(df.format(pNoBosque));
			Double pSinInfo = Util.calcularPorcentaje(total.intValue(), totalSinInfo.intValue());
	...
	private void poblarReporteWebConsolidado(Integer[] idReporte, String[] periodo) {
		Double totalBosque = new Double(0);
		Double totalNoBosque = new Double(0);
		Double totalSinInfo = new Double(0);
		Double total = new Double(0);
	...
	private void insertarImagenConsolidado(JFreeChart chart, int grafica, int columna, int hoja) {
	...
	private void poblarReporte(Integer[] idReporte) {
		...
		for (int i = 0; i < idReporte.length; i++) {
			ArrayList<InformacionReporteBosque> info = car.consultarInfoBosque(idReporte[i]);
			int cont = 0;
			for (int j = 0; j < info.size(); j++) {
				InformacionReporteBosque b = info.get(j);
				XSSFRow fila = libroReporte.getFilas()[cont];
				XSSFRow fila1 = libroReporte.getFilas()[cont + 1];
				Double total = b.getBosque() + b.getNoBosque() + b.getSinInformacion();
				...
				Double pb = Util.calcularPorcentaje(total.intValue(), b.getBosque().intValue());
				...
				Double pnb = Util.calcularPorcentaje(total.intValue(), b.getNoBosque().intValue());
				...
				Double psi = Util.calcularPorcentaje(total.intValue(), b.getSinInformacion().intValue());
	...
	/* public static void main(String[] a) { String p[] = { "2000" }; ConsultarReporteBosqueNoBosque v = new ConsultarReporteBosqueNoBosque(); Integer r[] = { 124 }; v.consultarReporte(4, p, r); } */
