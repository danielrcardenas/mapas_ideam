...
/**
 * Clase usada para generar el reporte xls de Deforestacion.
 */
@Stateless
public class CrearLibroReporteDeforestacion {
	...
	private void contarRegiones(
		...
		porcentajeAreasUrbanizadas=calcularPorcentajes(porcentajeAreasUrbanizadas);
		porcentajeCultivosTransitorios=calcularPorcentajes(porcentajeCultivosTransitorios);
		porcentajeCultivosPermanentes=calcularPorcentajes(porcentajeCultivosPermanentes);
		porcentajePastos=calcularPorcentajes(porcentajePastos);
		porcentajeAreaAgricolas=calcularPorcentajes(porcentajeAreaAgricolas);
		porcentajeHerbazales=calcularPorcentajes(porcentajeHerbazales);
		porcentajeArbustales=calcularPorcentajes(porcentajeArbustales);
		porcentajeVegetacionSecundaria=calcularPorcentajes(porcentajeVegetacionSecundaria);
		porcentajeZonasQuemadas=calcularPorcentajes(porcentajeZonasQuemadas);
		porcentajeOtrasAreas=calcularPorcentajes(porcentajeOtrasAreas);
		porcentajeVegetacionAcuatica=calcularPorcentajes(porcentajeVegetacionAcuatica);
		porcentajeSuperficieAgua=calcularPorcentajes(porcentajeSuperficieAgua);
	...
	public String[] calcularPorcentajes(String[] arreglo){
		String[] resultado=new String[arreglo.length];
		
		Float suma= 0f;
		Float aux=0f;
		for(int i=0;i<arreglo.length;i++){
			if(arreglo[i]!=null && !arreglo[i].equals("")){
				suma=suma+Float.parseFloat(arreglo[i]);
			}
		}
		
		for(int i=0;i<arreglo.length;i++){
			if(arreglo[i]!=null && !arreglo[i].equals("")){
				aux= (Float.parseFloat(arreglo[i])/suma)*100f;
				resultado[i]=aux.toString();
			}else{
				resultado[i]="";
			}
