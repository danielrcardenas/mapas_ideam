...
public class ActualizarUsuario {
	...
	public boolean modificarUsuario() {
		...
		try {
			...
			CallableStatement actualizarUsuario = conn
					.prepareCall("{call RED_PK_USUARIOS.Usuario_Actualiza(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			...
			actualizarUsuario.setInt("un_TipoIdentificacion",
					un_TipoIdentificacion);
			actualizarUsuario.setString("una_Identificacion",
					una_Identificacion);
			...
			actualizarUsuario.setString("un_TelefonoOficina",
					un_TelefonoOficina);
			actualizarUsuario.setString("un_Celular", un_Celular);
			actualizarUsuario.setString("un_CorreoElectronico",
					un_CorreoElectronico);
			if (!una_Clave.equals("")) {
				actualizarUsuario.setString("una_Clave",
			...
			actualizarUsuario.registerOutParameter("un_Resultado",
					OracleTypes.VARCHAR);
			actualizarUsuario.registerOutParameter("una_Respuesta",
					OracleTypes.NUMBER);
			...
			if (Integer.valueOf(actualizarUsuario.getInt("una_Respuesta")) == 1) {
				...
				Usuario auxUs = consUs.consultarUsuarioPorDoc(Integer
						.valueOf(una_Identificacion));
	...
	private void actualizarPersonaApollo(Usuario u) {
		try {
			...
			CallableStatement usuarioapollo = conn
					.prepareCall("{call RED_PK_APOLLO.ActualizarUsuarioApollo(?,?,?,?,?,?,?,?,?)}");
	...
	public boolean modificarUsuario(String[] rol) {
		...
		try {
			...
			perfil = null;
			if (perfil != null && !roles.isEmpty())
				insertarActividadPersona();
			if (rol != null)
				insertarRolPersona(rol);
	...
	public void modificarClaveUsuario(String clavePlainText, int document) {
		try {
			log = SMBC_Log.Log(ActualizarUsuario.class);
			conn = conexion.establecerConexion();
			CallableStatement actualizarClave = conn
					.prepareCall("{call RED_PK_USUARIOS.Usuario_Actualiza_Clave(?,?,?)}");
	...
	public void modificarRolUsuario(int un_Rol,int idUs) {
		try {
			log = SMBC_Log.Log(ActualizarUsuario.class);
			conn = conexion.establecerConexion();
			CallableStatement actualizarRol = conn
					.prepareCall("{call RED_PK_USUARIOS.Usuario_Actualiza_Rol(?,?)}");
	...
	public void insertarActividadPersona() {
		try {
			log = SMBC_Log.Log(ActualizarUsuario.class);
			conn = conexion.establecerConexion();
			CallableStatement eliminarActividadUsuario = conn
					.prepareCall("{call RED_PK_USUARIOS.ActividadPersona_Elimina(?,?)}");
			eliminarActividadUsuario.setInt("una_Persona",
					perfil.getIdperfilusuario());
			eliminarActividadUsuario.registerOutParameter("un_Resultado",
					OracleTypes.VARCHAR);
			eliminarActividadUsuario.execute();
			eliminarActividadUsuario.close();
			for (int i = 0; i < roles.size(); i++) {
				CallableStatement actividadPersona = conn
						.prepareCall("{call RED_PK_USUARIOS.ActividadPersona_Inserta(?,?,?)}");
				actividadPersona.setInt("un_perfil",
						perfil.getIdperfilusuario());
	...
	private void insertarLicenciasPersona(Integer idUsuario) {
		try {
			...
			eliminarLicencias = conn
					.prepareCall("{call RED_PK_USUARIOS.LicenciaPersona_Elimina(?,?)}");
			eliminarLicencias.setInt("una_Persona", idUsuario);
			eliminarLicencias.registerOutParameter("un_Resultado",
					OracleTypes.VARCHAR);
			...
			for (int i = 0; i < licencia.size(); i++) {
				...
				System.out
						.println(registrarLicencias.getObject("un_Resultado"));
    ...
    public void activarUsuario(int idUsuario){
    	try {
			...
			conn = conexion.establecerConexion();
			activacion = conn
					.prepareCall("{call RED_PK_USUARIOS.UsuarioActivar(?,?)}");
			activacion.setInt("un_Id", idUsuario);
			activacion.registerOutParameter("un_Resultado",
					OracleTypes.VARCHAR);
			activacion.execute();
    ...
    public void desactivarUsuario(int idUsuario){
    	try {
			...
			conn = conexion.establecerConexion();
			activacion = conn
					.prepareCall("{call RED_PK_USUARIOS.UsuarioDesactivar(?,?)}");
			activacion.setInt("un_Id", idUsuario);
			activacion.registerOutParameter("un_Resultado",
					OracleTypes.VARCHAR);
			activacion.execute();
